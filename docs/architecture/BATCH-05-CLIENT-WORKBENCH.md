...
- `POST /api/v1/sessions/{session_id}/attachments` 接受 base64 内容，拒绝路径型文件名，并将附件隔离到工作区 `.devpilot/attachments/<session_id>/`。
...
- `uv --cache-dir .uv-cache run devpilot doctor`：通过。
...
uv --cache-dir .uv-cache run devpilot session chat <session-id> "分析这个问题"
```
