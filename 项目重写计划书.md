...
CodeAssist 2.0 是一个面向开发者的多智能体研发助手，也是一个面向个人与团队的跨平台 AI 编程 Agent。除通用对话外，系统必须能够自动解析代码仓库、定位 Bug、创建隔离开发任务、调度测试、汇总证据并生成 PR 文档。项目采用本地优先、服务化内核设计，同一套 Agent 服务同时为 CLI、桌面 GUI 和 Web 提供能力。系统支持 OpenAI、Anthropic API、多模态消息、工具调用、MCP、多 Agent、项目记忆、长期记忆、多用户、多项目、团队权限和会话共享。

首发平台为 Windows 和 Linux；macOS 从架构、代码和构建流程上保持兼容，但不作为首期强制验收平台。网页端首先实现远程访问 Agent、项目文件、终端和审批流程。桌面画面、鼠标键盘及应用控制只预留安全接口和 TODO，不在首期实现。
...
- 提供 CLI、Tauri 桌面 GUI、浏览器 Web 三种客户端。
- 桌面端采用 Tauri 2 + React + TypeScript，并复用 Web 前端组件。
...
- Windows、Linux 完整运行；macOS 保持可编译、可移植的扩展能力。
...
| Web | React + TypeScript + Vite | 作为浏览器端和桌面端共享 UI 基础 |
| 桌面端 | Tauri 2 | 提供安装包、系统托盘、本地服务管理和受控原生能力 |
...
1. **本地模式**：桌面端启动本机 FastAPI Sidecar；CLI、GUI 和浏览器访问同一服务。
...
领域包不得直接依赖 FastAPI、Tauri 或具体数据库。客户端只调用公开 API，不直接操作 Agent 内部状态。研发工作流通过 `contracts` 交换结构化任务、证据、测试结果和 PR 文档，不把这些内容拼接成不可追踪的自然语言文本。
...
这些对象都要进入数据库或事件日志，能够从 PR 文档中的每条结论追溯到对应的 diff、测试事件或仓库证据。
...
Tauri 原生命令采用最小权限能力配置。前端不得直接获得任意 Shell 或文件系统权限。
...
- 首发完整支持。
- Tauri 使用 WebView2，提供 `.msi` 或 NSIS 安装包。
- Shell 默认 PowerShell，保留 `cmd` 和 Git Bash 配置。
- 使用 Windows Credential Manager 保存敏感凭据。

### Linux

- 首发完整支持 Ubuntu LTS；其他发行版尽力兼容。
- 提供 AppImage 和 `.deb`，后续增加 RPM。
- Shell 默认由系统配置决定，凭据优先使用 Secret Service。

### macOS

- Tauri 本身支持 macOS，因此架构无需放弃兼容。
- 使用 POSIX 路径抽象、跨平台进程接口和 Keychain 适配器。
- CI 至少运行 Python/前端测试；安装包、签名和公证作为后续里程碑。
...
AgentRun, RunEvent, Checkpoint, Approval
...
- Windows/Linux 执行 CLI、API、Web 和 Tauri 冒烟测试。
...
- 完成空壳 CLI、FastAPI、React 和 Tauri 连通。

验收：四个应用可启动，CI 在 Windows/Linux 通过。
...
验收：使用样例缺陷仓库完成“发现问题 → 定位 → 修复 → 测试 → PR 文档”的端到端流程，所有结论可追溯到证据。
...
- Tauri 管理本地服务、目录、托盘、通知和密钥库。
- 构建 Windows、Linux 安装包；验证 macOS CI 兼容。

验收：Windows/Linux 全新机器可安装并完成首次对话。
...
| Tauri 打包 Python 服务复杂 | 将 FastAPI 构建为平台 Sidecar；按平台 CI 构建并签名 |
...
| 跨平台 Shell 差异 | 平台适配器、能力探测和 Windows/Linux 分别测试 |
...
- [Tauri 分发文档](https://v2.tauri.app/distribute/)：支持 Windows、Linux 和 macOS 的平台安装包与签名流程。
